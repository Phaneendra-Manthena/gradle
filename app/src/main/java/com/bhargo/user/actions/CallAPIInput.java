...
public class CallAPIInput {
    ...
    HashMap<String, String> lkey_input_map = new HashMap<>();
    HashMap<String, String> lkey_map_input = new HashMap<>();
    ExpressionMainHelper ehelper;

    public CallAPIInput(Context context, ActionWithoutCondition_Bean actionBean, GetAPIDetails_Bean getAPIDetails_bean) {
    ...
    private void getAllDataSources_Filters_ParentType(List<API_InputParam_Bean> list_input) {
        ...
        for (int i = 0; i < list_input.size(); i++) {
            ...
            if (dataSourceName != null && !dataSourceName.trim().isEmpty()) {
                ...
                l_dataSources_filtered.put(apiInParam, getDataSourceData(dataSourceName));
    ...
    public JSONObject getInputHashMapObj(List<API_InputParam_Bean> list_input, String SuccessCaseDetails) {
        ...
        try {
            ...
            jArrayKeys = BodyObject.getJSONArray("postInputRawTypeContentKeys");
            //Convert json Obj to Sample HashMap
            HashMap<String, Object> sampleHashMapObj = JsonBuilder.ConvertJSONToHashMap(jsonString);
            String updateJObjData_ = JsonBuilder.buildJsonFromHashMap(inputobj, jsonSchemaString);
            for (int i = 0; i < list_input.size(); i++) {
                API_InputParam_Bean inputParamBeam = list_input.get(i);
                String inParamName = inputParamBeam.getInParam_Name();
                ...
                if (!isParentAvailable) {
                    ...
                    if (parentKeys.length - 1 == 1) {
                        ...
                        levelOne(inputobj, parentKeys, static_, expressionOrValue, apiInParam, inparamType);
    ...
    private void levelOne(HashMap<String, Object> inputobj, String[] parentKeys, String static_,
        ...
        if (l_keyType.get(parentKeys[0]).toString().equalsIgnoreCase("JArray")) {
            setLastLevel_jArray(inputobj, level_keypos, parentKeys[0], expressionOrValue, apiInParam, inparamType);
        } else {//JObject
    ...
    private void levelTwo(HashMap<String, Object> inputobj, String[] parentKeys, String static_,
        ...
        if (l_keyType.get(parentKeys[0]).toString().equalsIgnoreCase("JArray")) {
            ...
            for (int l1 = 0; l1 < parentData_l1.size(); l1++) {//top parent level 1 loop
                ...
                level_keypos.put("parentIndex",l1);
            ...
            for (int l1 = 0; l1 < parentData_l1.size(); l1++) {//top level 1 parent loop
                ...
                if (l_keyType.get(parentKeys[1]).toString().equalsIgnoreCase("JArray")) {
                    ...
                    for (int l2 = 0; l2 < parentData_l2.size(); l2++) {// level 2 parent loop
                        ...
                        level_keypos.put("parentIndex",l2);
                    ...
                    for (int l2 = 0; l2 < parentData_l2.size(); l2++) {// level 2 parent loop
                        ...
                        if (l_keyType.get(parentKeys[2]).toString().equalsIgnoreCase("JArray")) {
                            ...
                            for (int l3 = 0; l3 < parentData_l3.size(); l3++) {// level 3 parent loop
                                ...
                                level_keypos.put("parentIndex",l3);
                            ...
                            for (int l3 = 0; l3 < parentData_l3.size(); l3++) {// level 3 parent loop
                                ...
                                if (l_keyType.get(parentKeys[3]).toString().equalsIgnoreCase("JArray")) {
                                    ...
                                    for (int l4 = 0; l4 < parentData_l4.size(); l4++) {
                                        ...
                                        level_keypos.put("parentIndex",l4);
                                    ...
                                    for (int l4 = 0; l4 < parentData_l4.size(); l4++) {// level 4 parent loop
                                        ...
                                        if (l_keyType.get(parentKeys[4]).toString().equalsIgnoreCase("JArray")) {
                                            ...
                                            for (int l5 = 0; l5 < parentData_l5.size(); l5++) {//level 5 parent loop
                                                ...
                                                level_keypos.put("parentIndex",l5);
    ...
    private LinkedHashMap<String, List<String>> getFilterData(LinkedHashMap<String, List<String>> dataSource,
        ...
        for (int f = 0; f < filters.size(); f++) {
            ...
            if (dataSource.containsKey(whereColName) && expression != null && !expression.isEmpty()) {
                expression = ImproveHelper.spilitandgetcolumnname(expression);
                if(l_dataSources.containsKey(getParentKey(parentKeyLastLevel))){
                    LinkedHashMap<String, List<String>> pDataSource = (LinkedHashMap<String, List<String>>) l_dataSources.get(getParentKey(parentKeyLastLevel));;
                    if(pDataSource.containsKey(expression)){
                        List<String> values = pDataSource.get(expression);
                        whereColVal = values.get(level_keypos.get(expression));
                    }
                }
/*                if (expression.startsWith("(im:")) {
                ...
                l_dataSources.put(parentKeyLastLevel+"$"+level_keypos.get("parentIndex"),dataSource);
            } else {
    ...
    private String getParentKey(String keyName){
        try {
            //keyName	:	StateID
            //path	:	States/StateID
            String path = "";
            for (int i = 0; i < jArrayKeys.length(); i++) {
                ...
                if (jsonObject.get("keyName").toString().equalsIgnoreCase(keyName)) {
                    ...
                    for (int j = 0; j < pathArray.length; j++) {
                        if (j > 0 && keyName.contentEquals(pathArray[j])) {
                            return pathArray[j - 1];
                        }
                    }
                    break;
                }
