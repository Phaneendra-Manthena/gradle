...
public class FileUploader_ML extends AsyncTask<String,Void,String> {

    private String title,userId,pageName;
    boolean forSave;
    private FileInputStream fileInputStream = null;
    private Context context;
    private String response,Labelstr;
...
boolean fromTesting;
    ...
    public FileUploader_ML(Context context, String title, String userId,String OrgID, String pageName
            , boolean forSave, String projectName, String Labelstr,boolean fromTesting,OnImageUploaded onImageUploaded){
    ...
    protected String doInBackground(String... strings) {
        try {
            ...
            String url="";
            if(!fromTesting){
                url= GetServices.FILE_UPLOAD_URL_ML;
            }else{
                url=GetServices.FILE_UPLOAD_URL_ML_TEST;
            }
            ...
            dos.writeBytes("Content-Disposition: form-data; name=\"OrgID\""
                    + lineEnd);
            dos.writeBytes(lineEnd);
            dos.writeBytes(OrgID);
            dos.writeBytes(lineEnd);
            dos.writeBytes(twoHyphens + boundary + lineEnd);
            ...
            dos.writeBytes(twoHyphens + boundary + lineEnd);
            if(!fromTesting) {
                dos.writeBytes("Content-Disposition: form-data; name=\"Lable\""
                        + lineEnd);
                dos.writeBytes(lineEnd);
                dos.writeBytes(Labelstr);
                dos.writeBytes(lineEnd);
    ...
    protected void onPostExecute(String s) {
        ...
        System.out.println("Fileupload Response=="+s);

        if(s != null && !s.contentEquals("")){
            if(s.split("\\$")[0].equalsIgnoreCase("Sucess")) {
                onImageUploaded.response(s.split("\\$")[1]);
            }else{
                ImproveHelper.showToast(context,s);
            }
