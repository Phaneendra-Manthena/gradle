...
public class OTPUtilsCopy {
    ...
    public OTPUtilsCopy(Context context, SessionManager sessionManager, Map<String, String> otpData, String fromActivity, ImproveDataBase improveDataBase, int FromAction, int Executeindex, IfElseBlock_Bean IfElseBlock_Bean) {
                                    ...
                                    for (int i = 0; i < postsMasterList.size(); i++) {
                                        ...
                                        if (partialDC != null && partialDC.size() > 0) {
                                            for (int j = 0; j < partialDC.size(); j++) {
                                                Log.d(TAG, "onResponsePartialDC: " + partialDC.get(j).getTextFilePath());
                                                new DownloadFilesFromURL(context, partialDC.get(j).getTextFilePath(), sessionManager, null, TAG, "",partialDC.get(j).getDataControlName(), new DownloadFilesFromURL.DownloadFileListener() {
                                                    ...
                                                    public void onFailed(String errorMessage) {
                                                        System.out.println("nk file errorMessage:" + errorMessage);
                                                    }
                                                });
...
//                        updateTokenInFirebase();
                        if (fromActivity.equalsIgnoreCase("OTPActivity")) {
